...
const FilterBox = ({ jobType, setJobType, exp, setExp }) => {
  const [filterDropdown, setFiterDropdown] = useState({
    jobType: false,
    experience: false,
  });

  const handleJobTypeChange = (index) => {
    if (index === 0) {
      setJobType({ ...jobType, "Full-Time": !jobType["Full-Time"] });
    } else if (index === 1) {
      setJobType({ ...jobType, "Part-Time": !jobType["Part-Time"] });
    } else {
  ...
  const handleExpChange = (index) => {
    let updatedExp = { ...exp };
    updatedExp[Object.keys(exp)[index]] = !exp[Object.keys(exp)[index]];
    setExp(updatedExp);
  };
    ...
    <div className="hidden lg:block bg-[#ffffff] dark:bg-blue-100 p-8 rounded shadow-md h-[500px]">
          ...
          {["Full-Time", "Part-Time", "Contract"].map((type, index) => (
            <li key={index} class="flex items-center mb-4">
              <input
                ...
                onChange={() => handleJobTypeChange(index)}
      ...
      <div>
        ...
        >
          ...
          <span className="text-left whitespace-nowrap">Experience</span>
        ...
        >
          {["0 - 1", "1 - 3", "3 - 6", "6 - 10", "10 - 20"].map(
            (type, index) => (
              <li key={index} class="flex items-center mb-4">
                <input
                  id={"exp" + index}
                  type="checkbox"
                  checked={exp[Object.keys(exp)[index]]}
                  onChange={() => handleExpChange(index)}
                  className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded"
                />
                <label
                  htmlFor={"exp" + index}
